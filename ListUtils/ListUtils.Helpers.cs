...
public partial class ListUtils
{
    ...
    private readonly record struct CompiledCondition(string[]? Path, string Value, string Operator, bool CaseSensitive);
    ...
    private static (string composite, string[] parts) BuildCompositeKey(JsonNode item, List<string> keyPaths)
    {
        int n = keyPaths?.Count ?? 0;
        if (n == 0) return (UnknownKey, System.Array.Empty<string>());
        ...
        return (sb.ToString(), parts);
    }

    // Pre-split overload — accepts a jagged array of path segments so the caller can compile once per action call.
    private static (string composite, string[] parts) BuildCompositeKey(JsonNode item, string[]?[] keySegments)
    {
