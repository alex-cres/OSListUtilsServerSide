...
| [ListUtils.Helpers.cs](ListUtils/ListUtils.Helpers.cs) | `GetPropertyValue` + `NavigateSegment` path walker (with a pre-split `string[]?` overload used by all per-row hot loops), `SplitPath` one-shot dotted-path splitter, `MatchesCondition` operator evaluator, `ParseConditions` + `EvaluateConditions` multi-condition engine (plus a `CompiledCondition` record struct + `CompileConditions` + a pre-compiled `EvaluateConditions(JsonNode, CompiledCondition[]?, string)` overload), `TryCompareNumeric` numeric comparator, `ToCamelCase` fallback, `BuildCompositeKey` (Unit-Separator-joined N-key composite with `"Unknown"` fallback, plus a pre-compiled `BuildCompositeKey(JsonNode, string[]?[])` overload fed by `CompileKeyPaths`) and `KeyLabel` (label lookup with `"Key{i}"` default) shared by the multi-key grouping actions, nested `Condition` type, shared `JsonSerializerOptions` |
...
| [Actions/ListUtilsActions.Helpers.cs](ListUtils.O11/Actions/ListUtilsActions.Helpers.cs) | Path walker (with a pre-split `string[]?` overload used by all per-row hot loops) + `SplitPath` one-shot dotted-path splitter, condition evaluator (`List<Condition>` typed input), multi-condition engine (plus a `CompiledCondition` struct + `CompileConditions` + pre-compiled `EvaluateConditions` overload), `TryCompareNumeric`, `ToCamelCase`, `BuildCompositeKey` + `KeyLabel` (shared by the multi-key grouping actions, plus a pre-compiled `BuildCompositeKey(JsonNode, string[]?[])` overload fed by `CompileKeyPaths`), shared `JsonSerializerOptions` |
